...
cPoints=[         0    0.1430    0.4290   16.7053   -3.1978    2.1208   -3.8964    9.0700    9.0700    9.0700;
         0    0.1430    0.4290   10.0478   -4.5167    5.4447  -17.5588   -1.0700   -1.0700   -1.0700;
    1.0000    1.1400    1.4300   -3.1796    0.8842    0.0540    8.0130   -1.2000   -1.2000   -1.2000];
num_points=200;

figure; hold on; title('position'); 
figure; hold on;title('velocity'); 
for i=4:(size(knots,2)-3)
[knots(i) knots(i+1)]

increm=0.001;
pos = bspline_deboor(n,knots,cPoints,knots(i):increm:knots(i+1) );%0:increm:1

vel=[diff(pos(1,:));  diff(pos(2,:));  diff(pos(3,:))]/increm;
accel=[diff(vel(1,:)) ; diff(vel(2,:)) ; diff(vel(3,:))]/increm;
figure(1); hold on;
plot3(pos(1,:),pos(2,:),pos(3,:))%,'-b'

figure(2); hold on;
plot3(vel(1,:),vel(2,:),vel(3,:))%,'-b'
